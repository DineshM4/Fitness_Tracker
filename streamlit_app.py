...
def fetch_all():
    ...
    data = response.json()
    return [
        (w['id'], w['exercise'], w['sets'],
         w['reps'], w['weight'], w.get('date', ''))
        for w in data
    ]


def create_workout(exercise, sets, reps, weight, workout_date):
    response = requests.post(f'{API_BASE}/workouts', json={
        'exercise': exercise, 'sets': sets, 'reps': reps,
        'weight': weight, 'date': workout_date
    })
    response.raise_for_status()


def update_workout(workout_id, exercise, sets, reps, weight, workout_date):
    response = requests.put(f'{API_BASE}/workouts/{workout_id}', json={
        'exercise': exercise, 'sets': sets, 'reps': reps,
        'weight': weight, 'date': workout_date
    })
    response.raise_for_status()
...
df = pd.DataFrame(rows, columns=["ID", "Exercise", "Sets", "Reps", "Weight (kg)", "Date"]) if rows else pd.DataFrame(
    columns=["ID", "Exercise", "Sets", "Reps", "Weight (kg)", "Date"])
...
if active == "create":
    ...
    with st.form("create_form"):
        ...
        if submitted:
            if exercise.strip():
                try:
                    create_workout(exercise.strip(), int(sets), int(reps),
                                   float(weight), workout_date.isoformat())
    ...
    with st.form("update_form"):
        workout_id = st.number_input(
            "Workout ID to update", min_value=1, step=1)
        ...
        if submitted:
            if exercise.strip():
                try:
                    update_workout(int(workout_id), exercise.strip(), int(sets),
                                   int(reps), float(weight), workout_date.isoformat())
    ...
    with st.form("delete_form"):
        workout_id = st.number_input(
            "Workout ID to delete", min_value=1, step=1)
    ...
    st.warning(
        "This will permanently delete every workout log. This cannot be undone.")
    with st.form("delete_all_form"):
        submitted = st.form_submit_button(
            "Yes, delete everything", type="primary")
        if submitted:
            try:
                delete_all_workouts()
